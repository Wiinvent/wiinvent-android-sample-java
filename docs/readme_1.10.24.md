...
Change log:
- Update SDK version to `1.10.24`.
- **Breaking change (rename API)**: đổi tên tham số `uid` → `userId` trong tất cả request data (`AdsRequestData`, `BannerAdsRequestData`, `DisplayBannerAdsRequestData`, `WelcomeAdsRequestData`) và builder method `.uid(...)` → `.userId(...)`. **Biến gửi lên backend vẫn giữ nguyên là `uid`** — chỉ đổi tên phía API của SDK.
...
- `userId` — id người dùng (gửi lên backend qua param `uid`).
...
#### 1. SDK
```gradle
implementation 'tv.wiinvent:wiinvent-sdk-android:1.10.24'
```

#### 2. Hướng dẫn cập nhật

##### 2.1 Đổi tên `uid` → `userId`

Áp dụng cho tất cả request data. Chỉ đổi tên method builder, **giá trị và param gửi lên backend (`uid`) không đổi**.

```java
// Trước
new AdsRequestData.Builder().uid("123123123") ...

// Sau (1.10.24)
new AdsRequestData.Builder().userId("123123123") ...
```

> Nếu khởi tạo request data bằng constructor trực tiếp (không qua `Builder`) thì đổi tên tham số `uid` → `userId`.

##### 2.2 Bổ sung tham số `adPendingTime`
...
AdsRequestData adsRequestData = new AdsRequestData.Builder()
        ...
        .streamId("999999")
        .transId("a23334343")
        .userId("123123123") // tham so update
        .userImpressionLimit(1)
        .adPendingTime(28)            // <- tham số mới
...
##### 2.3 Report button và Info button
...
##### 2.4 Display Banner — bỏ report button khỏi layout và `requestAds`
...
##### 2.5 Overlay Banner — bỏ report button, SDK tự dựng skip button
...
##### 2.6 Welcome Ads — bổ sung `infoAdsButtonId`
...
##### 2.7 InStream Ads — đổi vị trí report button
...
|:-------------------------------------------|:-------------|:-----------------------------------------------------------------------------|
| `layout/layout_info_ads_button.xml`        | **Thêm mới** | Layout cho custom info button (xem mục 2.3). Chỉ cần khi tự custom `InfoButtonAds`. |
...
| Layout player InStream                     | Cập nhật     | Report button chuyển sang trái (xem mục 2.7)                                  |
| Layout Display Banner                      | Cập nhật     | **Xoá** report button khỏi layout (xem mục 2.4)                               |
| Layout item Display Banner (recycler/list) | Cập nhật     | **Xoá** report button khỏi layout item                                        |
| Layout Overlay Banner                      | Cập nhật     | **Xoá** report button và skip button khỏi layout (xem mục 2.5)                |
...
|:----------------|:----------------------------------------------------------------------------------|--------:|
| userId              | id người dùng. Gửi lên backend qua param `uid` (tên param backend không đổi).  |  string |
...
| Manager               | Thay đổi                                                                       |
|:----------------------|:-------------------------------------------------------------------------------|
| InStreamManager       | Đổi `uid` → `userId`. Request data thêm `adPendingTime`. Signature `requestAds(...)` không đổi, chỉ đổi vị trí report button trong layout. |
| BannerManager         | Đổi `uid` → `userId`. Request data thêm `adPendingTime`. Signature `requestAds(...)` không đổi.       |
| DisplayBannerManager  | Đổi `uid` → `userId`. Request data thêm `adPendingTime`. `requestAds(...)` **bỏ** `reportButton`.     |
| OverlayBannerManager  | Đổi `uid` → `userId`. Request data thêm `adPendingTime`. `requestAds(...)` và `releaseBanner(...)` **bỏ** `reportButton`. |
| AdsWelcomeManager     | Đổi `uid` → `userId`. Request data thêm `adPendingTime`. `requestAds(...)` **thêm** `infoAdsButtonId`. |
